...
export const getRulesByTenant = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const createRule = async (
  ...
  try {
    ...
    if (isFirstRule) {
      ...
      const lastRule = await RuleModel.findOne({ tenantId }).sort({
        ruleIndex: -1,
      });
...
export const updateRule = async (
  ...
  try {
    ...
    if (rule.tenantId !== tenantId) {
      return next(
        Object.assign(new Error("Access denied to this rule"), {
          statusCode: 403,
        }),
      );
    }
...
export const reorderRule = async (
  ...
  try {
    ...
    const getIndex = async (ruleId: string | undefined, label: string) => {
      ...
      if (!r) {
        throw Object.assign(new Error(`${label} not found`), {
          statusCode: 400,
        });
      }
      return r.ruleIndex;
    ...
    if (beforeRuleIndex !== null && afterRuleIndex !== null) {
      ...
      const maxIndex = await RuleModel.find({ tenantId: rule.tenantId })
        .sort({ ruleIndex: -1 })
        .limit(1)
    ...
    if (newIndex <= 0) {
      return next(
        Object.assign(new Error("Cannot place rule below the Cleanup Rule"), {
          statusCode: 400,
        }),
      );
    }
